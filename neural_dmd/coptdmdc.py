...
def run(snap: dict, *, rank: int | None = None, pod_rank: int | None = None,
    ...
    # Decoupled outer / inner LM driver (mirror of optdmdc.run): outer
    # = accepted iterations (capped by max_iter); inner = trust-region
    # damping retries (capped by gmax). Jacobian + scales recomputed
    # only on accepted moves. The radial-projection clip is applied to
    # every trial gamma so the constraint is honored even on rejected
    # trials.
    ...
    while iters_done < max_iter:
        ...
        for inner in range(gmax + 1):
            ...
            # Constraint already enforces Re(gamma_new) <= 0; the
            # radial-projection clip below is a numerical safety net
            ...
            nu *= incr
            log("warn",
                f"cOptDMDc: iter {iters_done + 1:3d}.{inner_restarts:<2d}  "
                f"residual rose ({res_t:.6e})  nu={nu:.3e}  "
                f"restarts={inner_restarts}/{gmax}")
        if not accepted:
            log("warn",
                f"cOptDMDc: gmax={gmax} consecutive restarts exhausted at "
        ...
        iters_done += 1
        log("info",
            f"cOptDMDc: iter {iters_done:3d}  residual={res_norm:.6e}  "
        ...
        if stopper.update(res_norm):
            log("ok",
                f"cOptDMDc: early-stopped at iter {iters_done} - {stopper.reason}")
